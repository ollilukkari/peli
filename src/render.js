...
export function drawGame(ctx, game, options = {}) {
  ...
  if (game.phase !== 'ready' && player.state !== 'petting') {
    // Screen-relative feet position: fade in across the bottom 20% as the camera moves.
    const danger = Math.max(0, Math.min(1, (bunnyY - HEIGHT * 0.8) / (HEIGHT * 0.2)));
    if (danger > 0) {
      ctx.save();
