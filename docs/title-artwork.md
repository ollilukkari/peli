...
The title in `index.html` is fixed artwork for the phrase **PUPUN / POMPUTTELU / PELI**, rendered from Cooper Black. Each complete word is an SVG path. The repository does not include or serve a font file, a converted font, or a reusable alphabet.

The three words have equal visible widths of 324 units, with their original letter proportions and 8-unit gaps. CSS scales the complete logo to 92% of the game frame, leaving 4% on either side. The middle word inherits the current world's accent color. A visually hidden text label preserves the accessible H1 heading.
